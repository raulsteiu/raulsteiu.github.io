...
**Platform version:** 3.1 — 2026-09-21  
...
└── clients/
    ...
    ├── story.js                    ← Shared runtime (v9.1) — fetched by every story page
    │
    └── [slug]/                     ← One folder per client story
        ...
        └── *.mp3                   ← Audio clip files (uploaded via edit mode)
```
...
This means:
- Fixing a bug in `story.js` propagates to **all** story pages instantly on next load
...
All write operations (save, delete, logo upload, audio upload) require a **fine-grained GitHub personal access token** with `Contents: read+write` permission on this repository.
...
Each language gets its own self-contained block. Non-EN content is stored under `data.translations[lc]`. Adding a language in edit mode clones the EN block with a `[FR]` prefix so translators know exactly what to update.
...
- **story.js changes propagate instantly** — no re-publish needed for any story after a runtime fix
